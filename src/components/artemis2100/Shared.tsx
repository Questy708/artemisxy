'use client';

import { useState } from 'react';
import { ChevronDown, Menu, X } from 'lucide-react';
import { clsx, type ClassValue } from 'clsx';
import { twMerge } from 'tailwind-merge';

export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
}

/* ──────────────────────────── Provocation Data ──────────────────────────── */

export const PROVOCATIONS = [
  {
    id: 'open-loop',
    title: 'Open Loop Learning',
    shortTitle: 'Open Loop',
    slug: 'open-loop',
    color: '#66B83B',
    bgGradient: 'bg-gradient-to-tr from-[#66B83B] via-[#7BCE44] to-[#4A9F2F]',
    hoverBg: 'hover:bg-[#5aa832]',
    description: 'We look back from 2100 at the era when Artemis brought an end to front-loaded education in favor of lifetime learning loops.',
    img: 'https://images.unsplash.com/photo-1516321497487-e288fb19713f?auto=format&fit=crop&q=80&w=2500',
    gridImg: 'https://images.unsplash.com/photo-1516321497487-e288fb19713f?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 'purpose-learning',
    title: 'Purpose Learning',
    shortTitle: 'Purpose',
    slug: 'purpose-learning',
    color: '#f2b90f',
    bgGradient: 'bg-[#f2b90f]',
    hoverBg: 'hover:bg-[#d6a40d]',
    description: 'A look back from 2100 to the era when Artemis students began declaring missions, not majors.',
    img: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&q=80&w=2500',
    gridImg: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 'paced-education',
    title: 'Adaptive Paced Learning',
    shortTitle: 'Paced',
    slug: 'paced-education',
    color: '#007f9c',
    bgGradient: 'bg-[#007f9c]',
    hoverBg: 'hover:bg-[#006e87]',
    description: 'In 2100, we examine the era when Artemis abolished the class year and embraced adaptive learning.',
    img: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&q=80&w=2500',
    gridImg: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 'skillprint',
    title: 'SkillPrint',
    shortTitle: 'SkillPrint',
    slug: 'skillprint',
    color: '#1a365d',
    bgGradient: 'bg-[#1a365d]',
    hoverBg: 'hover:bg-[#15294a]',
    description: 'A view from 2100 of the era when Artemis replaced transcripts with living skill-prints.',
    img: 'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&q=80&w=2500',
    gridImg: 'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 'centers-of-inquiry',
    title: 'Centers of Inquiry',
    shortTitle: 'Centers',
    slug: 'centers-of-inquiry',
    color: '#461e68',
    bgGradient: 'bg-[#461e68]',
    hoverBg: 'hover:bg-[#391854]',
    description: 'Looking back from 2100 at the radical experiment that replaced departments with interdisciplinary inquiry hubs.',
    img: 'https://images.unsplash.com/photo-1497215728101-856f4ea42174?auto=format&fit=crop&q=80&w=2500',
    gridImg: 'https://images.unsplash.com/photo-1497215728101-856f4ea42174?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 'infinite-learning',
    title: 'Infinite Learning Continuum',
    shortTitle: 'Infinite',
    slug: 'infinite-learning',
    color: '#d92231',
    bgGradient: 'bg-[#d92231]',
    hoverBg: 'hover:bg-[#b91d29]',
    description: 'From 2100, we reflect on the era when Artemis ended graduation and began the infinite continuum.',
    img: 'https://images.unsplash.com/photo-1509062522246-3755977927d7?auto=format&fit=crop&q=80&w=2500',
    gridImg: 'https://images.unsplash.com/photo-1509062522246-3755977927d7?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 'calibrate-elevate',
    title: 'Calibrate \u2192 Elevate \u2192 Activate',
    shortTitle: 'C\u2192E\u2192A',
    slug: 'calibrate-elevate',
    color: '#4338ca',
    bgGradient: 'bg-[#4338ca]',
    hoverBg: 'hover:bg-[#3730a3]',
    description: 'A retrospective from 2100 on the three-phase model that redefined how learners progress.',
    img: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&q=80&w=2500',
    gridImg: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&q=80&w=800',
  },
];

/* ──────────────────────────── NavBar ──────────────────────────── */

interface NavBarProps {
  currentPage: string;
  goTo: (page: string) => void;
  onExit: () => void;
}

export function NavBar({ currentPage, goTo, onExit }: NavBarProps) {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [isExploreOpen, setIsExploreOpen] = useState(false);

  const provocationSlugs = PROVOCATIONS.map(p => p.slug);
  const isExplorePath = provocationSlugs.includes(currentPage);

  return (
    <header className="flex items-center justify-between px-8 py-6 w-full max-w-6xl mx-auto relative z-50">
      <div className="flex items-center gap-2">
        <button onClick={() => goTo('home')} className="text-2xl font-semibold italic tracking-tight text-gray-900 cursor-pointer">
          Artemis<span className="font-light">2100</span>
        </button>
      </div>

      {/* Desktop Nav */}
      <nav className="hidden md:flex items-center gap-8 text-[11px] font-bold tracking-widest text-gray-400 uppercase">
        <button
          onClick={() => goTo('home')}
          className={cn("hover:text-gray-900 transition-colors cursor-pointer", currentPage === 'home' && "text-gray-900 border-b border-gray-900 pb-1")}
        >
          Home
        </button>
        <div
          className="relative group cursor-pointer"
          onMouseEnter={() => setIsExploreOpen(true)}
          onMouseLeave={() => setIsExploreOpen(false)}
        >
          <span className={cn("flex items-center gap-1 hover:text-gray-900 transition-colors pb-1", isExplorePath && "text-gray-900 border-b border-gray-900")}>
            Explore <ChevronDown className="w-3 h-3" />
          </span>
          {isExploreOpen && (
            <div className="absolute top-full right-0 pt-4">
              <div className="bg-white border border-gray-100 shadow-xl shadow-black/5 rounded flex flex-col min-w-[260px] overflow-hidden whitespace-nowrap">
                {PROVOCATIONS.map((p) => (
                  <button
                    key={p.slug}
                    onClick={() => { goTo(p.slug); setIsExploreOpen(false); }}
                    className={cn(
                      "px-6 py-4 hover:bg-gray-50 transition-colors text-xs text-gray-600 text-left cursor-pointer",
                      currentPage === p.slug && "font-bold text-gray-900"
                    )}
                  >
                    {p.title}
                  </button>
                ))}
              </div>
            </div>
          )}
        </div>
        <button
          onClick={() => goTo('build')}
          className={cn("hover:text-gray-900 transition-colors cursor-pointer", currentPage === 'build' && "text-gray-900 border-b border-gray-900 pb-1")}
        >
          Build
        </button>
        <button
          onClick={() => goTo('about')}
          className={cn("hover:text-gray-900 transition-colors cursor-pointer", currentPage === 'about' && "text-gray-900 border-b border-gray-900 pb-1")}
        >
          About
        </button>
        <button
          onClick={onExit}
          className="hover:text-gray-900 transition-colors cursor-pointer text-[#8A0000]"
        >
          Back to Artemis
        </button>
      </nav>

      {/* Mobile Toggle */}
      <button className="md:hidden" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
        {isMobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
      </button>

      {/* Mobile Nav */}
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-white shadow-xl shadow-black/10 flex flex-col p-6 md:hidden gap-6 font-bold text-sm tracking-widest text-gray-400 uppercase">
          <button onClick={() => { goTo('home'); setIsMobileMenuOpen(false); }}>Home</button>
          <div className="space-y-4">
            <span className="text-gray-900 border-b border-gray-900 pb-1 w-fit">Explore</span>
            <div className="flex flex-col gap-4 pl-4 border-l border-gray-100">
              {PROVOCATIONS.map((p) => (
                <button key={p.slug} onClick={() => { goTo(p.slug); setIsMobileMenuOpen(false); }} className="text-left">
                  {p.title}
                </button>
              ))}
            </div>
          </div>
          <button onClick={() => { goTo('build'); setIsMobileMenuOpen(false); }}>Build</button>
          <button onClick={() => { goTo('about'); setIsMobileMenuOpen(false); }}>About</button>
          <button onClick={() => { onExit(); setIsMobileMenuOpen(false); }} className="text-[#8A0000]">Back to Artemis</button>
        </div>
      )}
    </header>
  );
}

/* ──────────────────────────── Footer ──────────────────────────── */

export function Footer() {
  return (
    <footer className="bg-[#171717] text-white py-16 text-center text-xs space-y-2 mt-auto w-full">
      <p>Copyright &copy; 2100 University of Artemis. All ideas experimental.</p>
      <p className="text-gray-400">This website was made possible by the courageous women and men who invented the future of education.</p>
    </footer>
  );
}

/* ──────────────────────────── ExploreAnotherFuture ──────────────────────────── */

interface ExploreAnotherFutureProps {
  currentPage: string;
  goTo: (page: string) => void;
}

export function ExploreAnotherFuture({ currentPage, goTo }: ExploreAnotherFutureProps) {
  const others = PROVOCATIONS.filter(p => p.slug !== currentPage);
  // Show up to 3 provocations + Build link
  const show = others.slice(0, 3);

  return (
    <div className="max-w-4xl mx-auto px-6 py-24 border-t border-gray-200 mt-24">
      <SectionHeading>Explore Another Future</SectionHeading>

      <div className="flex flex-wrap justify-center gap-4 mt-12 w-full text-center">
        {show.map((p) => (
          <button
            key={p.slug}
            onClick={() => goTo(p.slug)}
            className={cn(
              "text-white font-bold w-48 h-24 flex items-center justify-center italic text-xl transition-colors cursor-pointer",
              p.bgGradient,
              p.hoverBg
            )}
          >
            <span className="leading-tight" dangerouslySetInnerHTML={{ __html: p.shortTitle.replace(' → ', '<br/>→ ') }} />
          </button>
        ))}
        <button
          onClick={() => goTo('build')}
          className="bg-[#d92231] hover:bg-[#b91d29] transition-colors text-white font-bold w-48 h-24 flex border-l-[8px] border-[#007f9c] items-center justify-center italic text-xl relative overflow-hidden group cursor-pointer"
        >
          <div className="absolute left-0 top-0 w-10 h-10 bg-yellow-400 -translate-x-5 -translate-y-5 rotate-45 group-hover:scale-110 transition-transform" />
          Design a<br/>Future
        </button>
      </div>
    </div>
  );
}

/* ──────────────────────────── SectionHeading ──────────────────────────── */

export function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="text-2xl font-bold tracking-[0.15em] uppercase text-gray-900 border-b border-black w-fit pr-12 pb-1">
      {children}
    </h2>
  );
}

/* ──────────────────────────── HeroHeader ──────────────────────────── */

export function HeroHeader({ title, description, bgGradientClass }: { title: string; description: string; bgGradientClass: string }) {
  return (
    <div className={cn("relative w-full h-[400px] overflow-hidden flex items-center", bgGradientClass)}>
      {/* Abstract background shapes */}
      <div className="absolute top-0 left-0 w-full h-full opacity-20 pointer-events-none">
        <div className="absolute top-[-10%] left-[10%] w-[500px] h-[500px] rounded-full border-[10px] border-white/40 border-dashed" />
        <div className="absolute top-[20%] right-[-10%] w-[600px] h-[600px] rounded-full border-[20px] border-white/20" />
      </div>

      <div className="max-w-5xl mx-auto px-6 relative z-10 w-full">
        <div className="bg-white p-8 md:p-12 w-full md:w-2/3 lg:w-[600px] border border-gray-100 shadow-xl relative mt-16">
          <h1 className="text-xl md:text-2xl font-bold tracking-[0.2em] uppercase text-gray-900 mb-6 border-b-2 border-black inline-block pb-1">
            {title}
          </h1>
          <p className="text-gray-600 text-sm md:text-base leading-relaxed">
            {description}
          </p>
        </div>
      </div>
    </div>
  );
}
