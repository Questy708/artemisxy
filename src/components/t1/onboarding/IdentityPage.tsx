...
export function IdentityPage({ onNext, travelerName, setTravelerName }: { onNext: () => void; travelerName: string; setTravelerName: (name: string) => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center w-full max-w-[360px] mx-auto">
      ...
      <h2 className="text-4xl font-medium text-gray-500 mb-6">Your portal to the future</h2>
      
      <p className="text-[15px] text-gray-600 mb-8 mt-2 leading-relaxed">
        Enter the Rift and begin<br />your exploration of the dimensions
      </p>
      ...
      <button className="w-full flex items-center justify-center space-x-3 bg-white border border-gray-200 rounded-full py-3.5 mb-6 hover:bg-gray-50 transition-colors">
        ...
        <span className="text-[15px] font-medium text-black">Enter as Explorer</span>
      </button>
      ...
      <div className="w-full relative">
         <input
           ...
           onFocus={() => setIsTyping(travelerName.length > 0)}
