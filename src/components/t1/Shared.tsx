...
/* ──────────────────────────── T1 Sub-NavBar ──────────────────────────── */
/* Sits below the Artemis Header as a secondary navigation bar,            */
/* aligned with the Artemis content width (max-w-[1400px]).                 */
...
export function T1NavBar({ currentPage, goTo, onExit }: NavBarProps) {
  ...
  return (
    <div className="w-full border-b border-gray-200 bg-white sticky top-0 z-40">
      <div className="max-w-[1400px] mx-auto w-full px-5 sm:px-8 lg:px-20 flex items-center justify-between py-4">
        {/* Logo / Title */}
        <button onClick={() => goTo('home')} className="text-xl font-semibold italic tracking-tight text-gray-900 cursor-pointer">
          Stanford<span className="font-light">2025</span>
        </button>

        {/* Desktop Nav */}
        <nav className="hidden md:flex items-center gap-6 text-[11px] font-bold tracking-widest text-gray-400 uppercase">
          <button
            onClick={() => goTo('home')}
          ...
          <span className="text-gray-300">Part 2 (New!)</span>
          <button onClick={onExit} className="hover:text-[#8A0000] transition-colors cursor-pointer text-[#8A0000]">&larr; Back to Artemis</button>
        </nav>
      ...
      {/* Mobile Nav */}
      {isMobileMenuOpen && (
        <div className="w-full bg-white border-t border-gray-100 flex flex-col p-6 md:hidden gap-4 font-bold text-sm tracking-widest text-gray-400 uppercase">
           ...
           <button onClick={() => { onExit(); setIsMobileMenuOpen(false); }} className="text-[#8A0000]">&larr; Back to Artemis</button>
        </div>
      )}
    </div>
  );
...
export function T1Footer() {
  return (
    <footer className="bg-[#171717] text-white py-12 text-center text-xs space-y-2 mt-auto w-full">
...
export function HeroHeader({ title, description, bgGradientClass }: { title: string; description: string; bgGradientClass: string }) {
  return (
    <section className="relative w-full overflow-hidden">
      <div className={cn("max-w-[1600px] mx-auto relative w-full h-[45vh] min-h-[360px] overflow-hidden flex items-center", bgGradientClass)}>
